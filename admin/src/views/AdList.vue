<template>
    ...
    <el-table :data="items">
        <!-- prop是数据的名称 数据库中的名称 -->
        <el-table-column prop="image" label="Banner" width="230">
        <template slot-scope="scope">
            <img :src="scope.row.items[0].image" alt="" style="height:3rem;">
        </template>
        </el-table-column>
...
export default {
    ...
    methods:{
        ...
        async remove(data)
        {
            ...
            const res= await this.$http.delete(`rest/ads/${data._id}`)
            // 重新获取数据
            this.fetch()
            this.$message({
            type: 'success',
