module.exports=app=>{
    ...
    //新闻导入接口
    router.get('/news/init',async(rq,rs)=>{
        ...
        const newsTitles= ["8月27日全服不停机更新公告", "乘风破浪闯峡谷 SNK永久英雄免费拿", "8月28日体验服停机更新公告", "8月28日体验服不停机更新公告", "8月27日体验服停机更新公告", "最强战队争霸赛参赛指南，如何让战队提升排名入围淘汰赛", "审判系统信誉经验恢复速率调整公告", "王者荣耀公布“无限开放计划”：探索游戏边界，共享荣耀", "暑期粉丝节狂欢，快手直播邀你一起来上分！", "《王者荣耀》对话岭南优秀非遗文化 跨界融合南狮发力新文创"]
    ...
    //前端调用
    router.get('/news/list',async(rq,rs)=>{
        ...
        //     }
        // }).lean()
        const pparent=await Category.findOne({
            name:"新闻分类"
        })
        //聚合查询
        const cats=await Category.aggregate([
            {$match:{parent:pparent._id}},
            {$lookup:{
                from:'articles',   //关联的表
                localField:"_id", //本地键
    ...
    //文章详情 不仅限于新闻
    router.get('/articles/:id',async(rq,rs)=>{
        const data=await Article.findById(rq.params.id).lean()
    ...
    //导入英雄数据
    router.get('/heros/init',async(rq,rs)=>{
        ...
        rs.send(await Hero.find())
        
    })

    //英雄显示
    router.get('/heroes/list',async(rq,rs)=>{
        ...
        //聚合查询 ！！！
        const cats=await Category.aggregate([
            {$match:{parent:pparent._id}}, //指定一个条件查询 =>where
            {$lookup:{
                from:'heroes',   //关联的表
                localField:"_id", //本地键
                foreignField:"categories", //外部键
    ...
    //英雄详细信息
    router.get('/heroes/:id',async(rq,rs)=>{
        const data=await Hero.findById(rq.params.id)
